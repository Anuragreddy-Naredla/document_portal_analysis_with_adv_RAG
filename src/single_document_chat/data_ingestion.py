...
class SingleDocIngestion:
    def __init__(self,data_dir="data/single_document_chat",faiss_dir="faiss_index"):
        try:
            ...
            self.log.info(f"SingleDoc Ingestion initialized",temp_path=str(self.data_dir),faiss_path=str(self.faiss_dir))
        except Exception as e:
            self.log.error(f"Error initialized in SingleDocIngestion: {e}")
    ...
    def _create_retrieval(self,documents):
        try:
            ...
            self.log.error(f"Document retriever failed",error=str(e))
